...
| `check-recovery.py` | `bash /root/dispatcharr-shell.sh shell < check-recovery.py`: whether Stream Recovery has ever forgiven a disconnect, and if not which of the four reasons it is — switched off, never asked, nothing in scope (is Plex recognised as a media server?), or declined as failing. Read-only. Run it while Plex is watching: question 3 only answers then. |
...
count at one; it needs rows that actually move, since the lookup only runs when a combine
changes a channel's group.
